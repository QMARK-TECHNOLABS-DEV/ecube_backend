...
class AdminGetExams(APIView):
    ...
    def get(self, request):
        try:
            ...
            cursor.execute(
                f"SELECT DISTINCT exam_name FROM public.{table_name} ORDER BY exam_name ASC;"
            )
        ...
        if not exam_name:
            if batch_year_cap is None or class_name_cap is None or division_cap is None:
                class_group_instance = (
                    class_details.objects.filter(exam_result__isnull=False)
                ...
                if not class_group:
                    ...
                    if not class_group.exam_result:
                        return Response(
                            {
                                "message": "No exam results published for this class group"
                            },
                            status=status.HTTP_400_BAD_REQUEST,
                        )
            ...
            cursor = connection.cursor()
            cursor.execute(
                f"SELECT admission_no, {subject} FROM public.{table_name} WHERE admission_no IS NOT NULL AND {subject} IS NOT NULL ORDER BY {subject} DESC NULLS LAST;"
            )
            query_results = cursor.fetchall()
            cursor.close()
            ...
            cursor = connection.cursor()
            cursor.execute(
                f"SELECT admission_no, {subject} FROM public.{table_name} WHERE exam_name = %s AND {subject} IS NOT NULL ORDER BY {subject} DESC NULLS LAST;",
                [exam_name],
            )
...
class GetLeaderBoard(APIView):
    def get(self, request):
        ...
        cursor = connection.cursor()
        cursor.execute(
            f"SELECT admission_no, {subject} FROM public.{table_name} WHERE admission_no IS NOT NULL AND {subject} IS NOT NULL ORDER BY {subject} DESC NULLS LAST LIMIT 10;"
        )
        query_results = cursor.fetchall()
        cursor.close()
...
class GetLeaderBoardByExams(APIView):
    def get(self, request):
        ...
        cursor = connection.cursor()
        cursor.execute(
            f"SELECT admission_no, {subject} FROM public.{table_name} WHERE exam_name = %s ORDER BY {subject} DESC NULLS LAST;",
            [exam_name],
        )
...
class RecalculateLeaderBoardByClass(APIView):
    ...
    def post(self, request):
        try:
            ...
            try:
                cursor.execute(
                    f"DELETE FROM public.{app_name}{app_name}{batch_year}_{class_name}_{division}_leaderboard;"
                )
