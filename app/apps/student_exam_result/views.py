...
class ExamResult(APIView, CustomPageNumberPagination):
    def post(self, request):
        try:
            ...
            if batch_year is None or class_name is None or division is None:
                return Response(
                    {"message": "batch_year, class_name and division is required"},
            ...
            if not isinstance(data, list):
                return Response(
                    {
                        "status": "failure",
                        "message": "Request body should be a JSON array",
                    },
                    status=status.HTTP_400_BAD_REQUEST,
                )
            ...
            for item in data:
                ...
                if admission_no is None or exam_name is None:
                    return Response(
                        {
                            "status": "failure",
                            "message": "admission_no and exam_name are required for each item",
                        },
                        status=status.HTTP_400_BAD_REQUEST,
                    )
                ...
                cursor.execute(
                    f"SELECT * FROM public.{table_name_examresults} WHERE admission_no = %s AND exam_name = %s",
                ...
                if query_results is None:
                    cursor.execute(
                        f"INSERT INTO public.{table_name_examresults} (admission_no, exam_name, physics, chemistry, maths, physics_status, chemistry_status, maths_status) VALUES (%s, %s, %s, %s, %s, %s, %s, %s)",
                        [
                            admission_no,
                            exam_name,
                            physics,
                    ...
                    cursor = connection.cursor()
                    cursor.execute(
                        f"SELECT * FROM public.{table_name_leaderboard} WHERE admission_no = %s",
                        [admission_no],
                    )
                    query_results = cursor.fetchone()
                    cursor.close()
                    ...
                    try:
                        ...
                        if query_results is None:
                            ...
                            cursor = connection.cursor()
                            cursor.execute(
                                f"INSERT INTO public.{table_name_leaderboard} (admission_no, physics, chemistry, maths) VALUES (%s, %s, %s, %s)",
                                [admission_no, physics, chemistry, maths],
                            )
                        else:
                            cursor = connection.cursor()
                            cursor.execute(
                                f"UPDATE public.{table_name_leaderboard} SET physics = physics + %s, chemistry = chemistry + %s, maths = maths + %s WHERE admission_no = %s",
                                [physics, chemistry, maths, admission_no],
                            )
                        ...
                        return Response(
                            {
                                "status": "failure",
                                "message": "admission_no, physics, chemistry, and maths are required",
                            },
                            status=status.HTTP_400_BAD_REQUEST,
                        )
            ...
            if student_list:
                ...
                message_desc_leaderboard = (
                    "Check the leaderboard now for current standings"
                )
                message_type_leaderboard = "leaderboard"

                send_notification_main(
                    device_ids, message_title_exam, message_desc_exam, message_type_exam
                )

                send_notification_main(
                    device_ids,
                    message_title_leaderboard,
                    message_desc_leaderboard,
                    message_type_leaderboard,
                )
    ...
    def put(self, request):
        try:
            ...
            for item in data:
                ...
                if query_results:
                    cursor.execute(
                        f"UPDATE public.{table_name_examresults} SET physics = %s, chemistry = %s, maths = %s WHERE admission_no = %s AND exam_name = %s",
                        [physics, chemistry, maths, admission_no, exam_name],
                    )
                    cursor.close()
                    ...
                    try:
                        if query_results:
                            ...
                            cursor = connection.cursor()
                            cursor.execute(
                                f"SELECT physics, chemistry, maths FROM public.{table_name_examresults} WHERE admission_no = %s",
                                [admission_no],
                            )
                            ...
                            cursor = connection.cursor()
                            cursor.execute(
                                f"UPDATE public.{table_name_leaderboard} SET physics = %s, chemistry = %s, maths = %s WHERE admission_no = %s",
                                [
                                    total_physics,
                                    total_chemistry,
                                    total_maths,
    ...
    def delete(self, request):
        try:
            ...
            for item in data:
                ...
                if query_results:
                    cursor.execute(
                        f"DELETE FROM public.{table_name_examresults} WHERE admission_no = %s AND exam_name = %s",
                        [admission_no, exam_name],
                    )
                    cursor.close()
    ...
    def get(self, request):
        try:
            ...
            if batch_year_cap is None or class_name_cap is None or division_cap is None:
                class_group_instance = (
                    class_details.objects.filter(
                        exam_result__isnull=False, exam_name__isnull=False
                    )
                    .order_by("-exam_result")
                    .first()
                )
            ...
            cursor = connection.cursor()
            cursor.execute(
                f"SELECT * FROM public.{table_name_examresults} WHERE exam_name = %s",
                [exam_type],
            )
            query_results = cursor.fetchall()
...
class GetExamType(APIView):
    def get(self, request):
        try:
            ...
            cursor = connection.cursor()
            cursor.execute(
                f"SELECT DISTINCT exam_name FROM public.{table_name_examresults}"
            )
            query_results = cursor.fetchall()
        ...
        try:
            ...
            for index, row in df.iterrows():
                ...
                if query_results is None:
                    ...
                    cursor.execute(
                        f"INSERT INTO public.{table_name_examresults} (admission_no, exam_name, physics, chemistry, maths) VALUES (%s, %s, %s, %s, %s)",
                        [admission_no, exam_name, physics, chemistry, maths],
                    )
    ...
    def get(self, request):
        ...
        try:
            ...
            for subject in subject_results:
                ...
                data_for_subject = [
                    {"exam_name": exam_name, "marks": marks}
                    for exam_name, marks in subject_data
            ...
            if not exams_data:
                return Response(
                    {
                        "status": "failure",
                        "message": "No exam results found for the specified admission_no",
                    },
                    status=status.HTTP_400_BAD_REQUEST,
                )
            ...
            response_data = {
                "no_of_exams": str(
                    total_exams
                ),  # Number of exams is the sum of all subject exams
                "results": exams_data,
            }
...
class AdminExamResultsView(APIView):
    ...
    def get(self, request):
        ...
        if not user_id:
            return Response(
                {"error": "The user id is not provided in the params."},
                status=status.HTTP_401_UNAUTHORIZED,
            )
        # Generate a new access token
        user = Student.objects.get(id=user_id)
