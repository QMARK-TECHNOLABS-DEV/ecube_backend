...
from .serializers import (
    recordings_get_serializer,
    class_updates_link_serializer,
    class_updates_link_get_serializer,
...
class Class_Updates_Admin(APIView, CustomPageNumberPagination):
    ...
    def post(self, request):
        ...
        print(data)
        class_group = class_details.objects.filter(
        ...
        if class_group == None:
            return Response(
                {"message": "Class not found"}, status=status.HTTP_400_BAD_REQUEST
            )
    ...
    def get(self, request):
        ...
        if class_name == None or batch_year == None or division == None:
            class_update_instance = class_updates_link.objects.order_by(
                "-upload_time"
            ).first()

            if not class_update_instance:
                return Response(
                    {"message": "No class links provided for any class group"},
        ...
        queryset = class_updates_link.objects.filter(
            class_name=class_name, batch_year=batch_year, division=division
    ...
    def delete(self, request):
        ...
        if link_id == None:
            ...
            queryset.delete()
            return Response(
                {"message": "Class Update Link deleted successfully"},
...
class Class_Update_Client_Side(APIView):
    def get(self, request):
        ...
        if user.subjects:
            subjects = [
                item.strip().upper()
                for item in user.subjects.split(",")
                if item.strip()
            ]
        ...
        if class_name == None or batch_year == None or division == None:
            ...
            queryset = class_updates_link.objects.filter(**filters).order_by(
                "-upload_time"
            )
            ...
            if serializer.data == []:
                return Response(
                    {"announcement": announcement, "class_links": serializer.data},
                    status=status.HTTP_200_OK,
                )
            else:
                return Response(
...
class GetRecordingDates(APIView):
    def get(self, request):
        try:
            ...
            division = user.division
            userSubjects = [
                s.strip().upper() for s in (user.subjects or "").split(",") if s.strip()
            ]
        ...
        try:
            ...
            if subject:
                ...
                if class_name == None or batch_year == None or division == None:
                    ...
                    recordings_instance = self.paginate_queryset(
                        recordings_instance, request
                    )

                    recording_serializer = recordings_get_serializer(
                        recordings_instance, many=True
                    )
...
class recording_client_side_web(APIView):
    def get(self, request):
        try:
            ...
            if not date:
                return Response(
                    {"message": "Date not found"}, status=status.HTTP_400_BAD_REQUEST
                )
...
class Announcements(APIView, CustomPageNumberPagination):
    ...
    def post(self, request):
        ...
        announcement_instance = announcements.objects.filter(
        ...
        if announcement_instance != None:
            ...
            return Response(
                {"message": "announcements updated succefully"},
                status=status.HTTP_200_OK,
            )
        else:

            announcements.objects.create(
                announcement=data["announcement"], upload_date=data["upload_date"]
            )

            return Response(
                {"message": "announcements added succefully"},
...
class RecordingsLink(APIView, CustomPageNumberPagination):
    def post(self, request):
        try:
            ...
            try:
                ...
                return Response(
                    {"error": "Invalid YouTube recording link."},
                    status=status.HTTP_400_BAD_REQUEST,
                )
            ...
            if students_instance:
                ...
                if device_ids:
                    ...
                    message_desc = (
                        f"Recorded class for {subject_display} on {date} has been added"
                    )
                    message_type = "liveclass"
            ...
            return Response(
                {"message": "Recordings link added successfully"},
                status=status.HTTP_201_CREATED,
            )
    ...
    def get(self, request):
        ...
        if class_name == None or batch_year == None or division == None:
            ...
            if not recordings_instance:
                return Response(
                    {"message": "No video links provided for any class group"},
                    status=status.HTTP_400_BAD_REQUEST,
                )
    ...
    def put(self, request):
        ...
        if recordings_instance:
            ...
            return Response(
                {"message": "Recordings link updated successfully"},
                status=status.HTTP_200_OK,
            )
        else:
            return Response(
                {"message": "Recordings link not found"},
                status=status.HTTP_400_BAD_REQUEST,
            )
    ...
    def delete(self, request):
        ...
        if recordings_id == None:
            ...
            queryset.delete()
            return Response(
                {"message": "Recordings link deleted successfully"},
                status=status.HTTP_200_OK,
            )
