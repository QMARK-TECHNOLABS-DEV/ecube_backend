...
class StudentFilterGetMethods(APIView):
    def get(self, request):
        ...
        if not serializer.data:
            return Response(
                {"Message": "No students found"}, status=status.HTTP_400_BAD_REQUEST
            )
...
class AllClassesView(APIView):
    def get(self, request):
        try:
            batch_year = request.data.get(
                "batch_year"
            )  # Use query_params for GET requests

            if not batch_year:
                unique_batch_years = (
                    class_details.objects.values_list("batch_year", flat=True)
                    .distinct()
                    .order_by("-batch_year")
                )
            ...
            if batch_year:
                ...
                return Response(
                    {"all_classes": serializer.data}, status=status.HTTP_200_OK
                )

            else:
                return Response(
                    {"Message": "Batch year not provided"},
                    status=status.HTTP_400_BAD_REQUEST,
                )
            ...
            print(e)
            return Response(
                {"Message": "Error in fetching classes"},
                status=status.HTTP_400_BAD_REQUEST,
            )
...
class AllBatchYear(APIView):
    def get(self, request):
        try:
            ...
            if not batch_year:
                return Response(
                    {"Message": "No classes found"}, status=status.HTTP_400_BAD_REQUEST
                )
...
class AllClassName(APIView):
    def get(self, request):
        try:
            class_name = (
                class_details.objects.values_list("class_name", flat=True)
                .distinct()
                .order_by("-class_name")
            )
...
class AllDivision(APIView):
    def get(self, request):
        try:
            division = (
                class_details.objects.values_list("division", flat=True)
                .distinct()
                .order_by("-division")
            )
