...
def send_notification_main(registration, message_title, message_desc, message_type):
    ...
    result = send_notification(
        registration, message_title, message_desc, message_type)
    print(result)
...
class DeleteAttendance(APIView):
    def delete(self, request):
        try:
            ...
            table_name = app_name + '_' + app_name + '_' + batch_year + \
...
class AddAttendance(APIView):
    def post(self, request):
        ...
        table_name = app_name + app_name + batch_year + \
            "_" + class_name + "_" + division + "_attendance"

        cursor = connection.cursor()
        cursor.execute(f"INSERT INTO public.{table_name} (admission_no, month_year_number, date, status,subject) VALUES (%s,%s, %s, %s, %s)", [
...
class AddAttendanceBulk(APIView):
    def post(self, request):
        ...
        class_instance = class_details.objects.filter(
            batch_year=batch_year, class_name=class_name, division=division).first()
        ...
        student_list = Student.objects.filter(
            class_name=class_name_notif, batch_year=batch_year_notif, division=division_notif).values('id', 'name', 'device_id')
        ...
        try:
            ...
            class_group_instance = class_details.objects.get(
                batch_year=batch_year_notif, class_name=class_name_notif, division=division_notif)
...
class AdminGetAttendanceMonth(APIView):
    def get(self, request):
        ...
        class_details_instance = class_details.objects.filter(
            batch_year=batch_year_cap, class_name=class_name_cap, division=division_cap).first()
        ...
        distinct_dates = []
        cursor = connection.cursor()
        cursor.execute(f"SELECT DISTINCT TO_DATE(date, 'DD/MM/YYYY') FROM public.{
                       table_name} WHERE subject = %s ORDER BY TO_DATE(date, 'DD/MM/YYYY') DESC;", [subject])
    ...
    def get(self, request):
        ...
        try:
            ...
            students_instance = Student.objects.filter(
                batch_year=batch_year_cap, class_name=class_name_cap, division=division_cap, subjects__contains=subject)
            ...
            for row in query_result:
                ...
                student = students_instance.filter(
                    admission_no=admission_no, subjects__contains=subject).first()
                if student:
                    attendance_entry = {
            ...
            try:
                attendance_data = self.paginate_queryset(
...
class AdminGetIndReportAttendanceMonth(APIView):
    def get(self, request):
        ...
        cursor = connection.cursor()
        cursor.execute(f"SELECT DISTINCT month_year_number FROM public.{
                       table_name} WHERE subject = %s ORDER BY month_year_number DESC;", [subject])
...
class AdminGetIndReportAttendance(APIView):
    def get(self, request):
        ...
        cursor.execute(f"SELECT * FROM public.{table_name} WHERE admission_no = %s AND month_year_number = %s AND subject = %s;", [
        ...
        cursor.execute(f"SELECT DISTINCT date FROM public.{
                       table_name} WHERE month_year_number = %s AND subject = %s;", [month_year_number, subject])
        ...
        present_days = [date_string_to_object(
            date) for date, status_att in attendance_data.items() if status_att == "P"]
        absent_days = [date_string_to_object(
            date) for date, status_att in attendance_data.items() if status_att == "A"]

        response_data = {"attendance_result": {
...
class GetAttendance(APIView):
    def get(self, request):
        ...
        for subject in subjects:
            cursor.execute(f"SELECT DISTINCT date FROM public.{
                           table_name} WHERE subject = %s AND month_year_number = %s;", [subject, month_year_number])
            rows = cursor.fetchall()  # Fetch all rows for the current subject
...
class GetAttendanceYearStatus(APIView):
    def get(self, request):
        ...
        try:
            ...
            for subject in subjects:
                print(subject)
                cursor.execute(f"SELECT * FROM public.{
                               table_name} WHERE admission_no = %s AND subject = %s;", [user.admission_no, subject])
            ...
            for subject in subjects:
                cursor.execute(f"SELECT DISTINCT date FROM public.{
                               table_name} WHERE subject = %s;", [subject])
                rows = cursor.fetchall()  # Fetch all rows for the current subject
            ...
            response_data = {"total_working_days": total_working_days, "present_days": len(
                present_days), "absent_days": len(absent_days)}
