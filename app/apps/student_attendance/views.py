...
class DeleteAttendance(APIView):
    def delete(self, request):
        try:
            ...
            cursor.execute(
                f"DELETE FROM public.{table_name} WHERE admission_no = %s;",
                [admission_no],
            )
            cursor.close()

            return Response(
                {"status": "successfully deleted"}, status=status.HTTP_200_OK
            )
    ...
    def post(self, request):
        ...
        if (
            ...
            return Response(
                {
                    "status": "failure",
                    "message": "admission_no, month_year_number, date and status are required",
                },
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        cursor.execute(
            f"INSERT INTO public.{table_name} (admission_no, month_year_number, date, status,subject) VALUES (%s,%s, %s, %s, %s)",
            [admission_no, month_year_number, date, status_student, subject],
        )
        cursor.close()
...
class AddAttendanceBulk(APIView):
    def post(self, request):
        ...
        if (
            ...
            or division is None
            or date_attendance is None
        ):
            return Response(
                {
                    "status": "failure",
                    "message": "batch_year, class_name, date and division are required",
                },
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        if class_instance is None:
            return Response(
                {"status": "failure", "message": "Class does not exist"},
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        try:
            ...
            if file.name.endswith(".csv"):
                ...
                return Response(
                    {"status": "failure", "message": "Unsupported file format"},
                    status=status.HTTP_400_BAD_REQUEST,
                )
        except Exception as e:
            return Response(
        ...
        try:
            for index, row in df.iterrows():
                ...
                if (
                    str(last_name) == ""
                    or str(last_name) == "nan"
                    or str(last_name) == " "
                    or str(last_name) == "None"
                ):
                    last_name = ""
                ...
                for student in student_list:
                    ...
                    if student_instance is not None:
                        ...
                        if (
                            admission_no is not None
                            and month_year_number is not None
                            and date is not None
                            and status_student is not None
                        ):
                            try:
                                cursor.execute(
                                    f"INSERT INTO public.{table_name} (admission_no, month_year_number, date, status,subject) VALUES (%s,%s, %s, %s, %s)",
            ...
            return Response(
                {
                    "status": "success",
                    "message": "Attendance added and notification send",
                },
...
class AdminGetAttendanceMonth(APIView):
    ...
    def get(self, request):
        ...
        class_details_instance = class_details.objects.filter(
            batch_year=batch_year_cap, class_name=class_name_cap, division=division_cap
        ).first()
        ...
        cursor = connection.cursor()
        cursor.execute(
            f"SELECT DISTINCT TO_DATE(date, 'DD/MM/YYYY') FROM public.{table_name} WHERE subject = %s ORDER BY TO_DATE(date, 'DD/MM/YYYY') DESC;",
            [subject],
        )
        ...
        response_data = {"distinct_dates": distinct_dates}
...
class AdminGetAttendance(APIView, CustomPageNumberPagination):
    ...
    def get(self, request):
        try:
            ...
            if batch_year_cap is None or class_name_cap is None or division_cap is None:
                class_group_instance = (
                    class_details.objects.filter(
                        attendance__isnull=False, attendance_date__isnull=False
                    )
                    .order_by("-attendance")
                    .first()
                )
            ...
            cursor = connection.cursor()
            cursor.execute(
                f"SELECT DISTINCT ON (admission_no) * FROM public.{table_name} WHERE date = %s AND subject = %s;",
                [query_date, subject],
            )
            ...
            for row in query_result:
                ...
                student = students_instance.filter(
                    admission_no=admission_no, subjects__contains=subject
                ).first()
                if student:
                    attendance_entry = {
            ...
            for student in students_instance:
                if not any(
                    entry["admission_no"] == student.admission_no
...
class AdminGetIndReportAttendanceMonth(APIView):
    ...
    def get(self, request):
        ...
        if user_id is None:
            return Response(
                {"status": "failure", "message": "user_id is required"},
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        if subject is None:
            return Response(
                {"status": "failure", "message": "subject is required"},
                status=status.HTTP_400_BAD_REQUEST,
            )
        elif subject not in subjects:
            return Response(
                {"status": "failure", "message": "subject does not exist"},
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        cursor = connection.cursor()
        cursor.execute(
            f"SELECT DISTINCT month_year_number FROM public.{table_name} WHERE subject = %s ORDER BY month_year_number DESC;",
            [subject],
        )
...
class AdminGetIndReportAttendance(APIView):
    ...
    def get(self, request):
        ...
        cursor.execute(
            f"SELECT * FROM public.{table_name} WHERE admission_no = %s AND month_year_number = %s AND subject = %s;",
            [user_instance.admission_no, month_year_number, subject],
        )
        query_result = cursor.fetchall()
        ...
        cursor.execute(
            f"SELECT DISTINCT date FROM public.{table_name} WHERE month_year_number = %s AND subject = %s;",
            [month_year_number, subject],
        )
        ...
        for row in rows:
            ...
            if (
                date not in distinct_dates
            ):  # Check if the date is not already in the list
                distinct_dates.append(
                    date
                )  # Add the date to the list if it's not already present
        ...
        present_days = [
            date_string_to_object(date)
            for date, status_att in attendance_data.items()
            if status_att == "P"
        ]
        absent_days = [
            date_string_to_object(date)
...
class GetAttendance(APIView):
    def get(self, request):
        ...
        for subject in subjects:
            cursor.execute(
                f"SELECT * FROM public.{table_name} WHERE admission_no = %s AND month_year_number = %s AND subject = %s;",
                [user.admission_no, month_year_number, subject],
            )
            query_result = cursor.fetchall()
        ...
        for subject in subjects:
            cursor.execute(
                f"SELECT DISTINCT date FROM public.{table_name} WHERE subject = %s AND month_year_number = %s;",
                [subject, month_year_number],
            )
            rows = cursor.fetchall()  # Fetch all rows for the current subject
    ...
    def get(self, request):
        ...
        try:
            ...
            for subject in subjects:
                print(subject)
                cursor.execute(
                    f"SELECT * FROM public.{table_name} WHERE admission_no = %s AND subject = %s;",
                    [user.admission_no, subject],
                )
            ...
            for subject in subjects:
                cursor.execute(
                    f"SELECT DISTINCT date FROM public.{table_name} WHERE subject = %s;",
                    [subject],
                )
                rows = cursor.fetchall()  # Fetch all rows for the current subject
