        ...
        try:
            ...
            if student_instance:
                ...
                if restricted_status is True or restricted_status == "true":
                    student_instance.device_id = (
                        ""  # To discontinue push notifications in mobile app
                    )

                student_instance.save()

                return Response(
                    {"message": "Student restriction updated and logout handled."},
            ...
            return Response(
                {"message": "Internal failure", "error": str(e)},
                status=status.HTTP_400_BAD_REQUEST,
            )
...
class NextAdmNumber(APIView):
    ...
    def get(self, request):
        try:
            highest_adm_no = Student.objects.annotate(
                admission_no_int=Cast("admission_no", IntegerField())
            ).aggregate(max_admission_no=Max("admission_no_int"))["max_admission_no"]

            print(highest_adm_no)

            if highest_adm_no:
                return Response(
                    {"next_adm_no": highest_adm_no + 1}, status=status.HTTP_200_OK
                )
...
class StudentMethods(APIView):
    ...
    def post(self, request):
        ...
        class_instance = class_details.objects.filter(
        ...
        if not class_instance:
            return Response(
                {"message": "No such class group"}, status=status.HTTP_400_BAD_REQUEST
            )
    ...
    def get(self, request):
        ...
        if serializer.data == []:
            return Response(
                {"Message": "The record or student does not exist"},
                status=status.HTTP_400_BAD_REQUEST,
            )
    ...
    def delete(self, request):
        ...
        if student.exists():
            student.delete()
            return Response(
                {"Message": "Record successfully deleted"}, status=status.HTTP_200_OK
            )

        return Response(
            {"Message": "Record does not  exist"}, status=status.HTTP_400_BAD_REQUEST
        )
    ...
    def put(self, request):
        ...
        class_instance = class_details.objects.filter(
            batch_year=new_batch_year, class_name=new_class_name, division=new_division
        )
        ...
        if students is not None:
            if (
                old_batch_year != new_batch_year
                or old_class_name != new_class_name
                or old_division != new_division
            ):

                new_table_name = (
                    f"{new_batch_year}_{new_class_name}_{new_division}".replace(
                        " ", ""
                    ).lower()
                )

                old_table_name = (
                    f"{old_batch_year}_{old_class_name}_{old_division}".replace(
                        " ", ""
                    ).lower()
                )
                ...
                final_new_table_name = (
                    f"{app_name}{app_name}{new_table_name}{feature_group_id[0]}"
                )
                final_old_table_name = (
                    f"{app_name}{app_name}{old_table_name}{feature_group_id[0]}"
                )
                ...
                with connection.cursor() as cur:
                    ...
                    try:
                        cur.execute(
                            f"INSERT INTO public.{final_new_table_name} (admission_no, exam_name, physics, chemistry, maths) SELECT admission_no, exam_name, physics, chemistry, maths FROM public.{final_old_table_name} WHERE admission_no = %s",
                            [students.admission_no],
                        )
                        cur.execute(
                            f"DELETE FROM public.{final_old_table_name} WHERE admission_no = %s",
                            [students.admission_no],
                        )
                ...
                final_new_table_name = (
                    f"{app_name}{app_name}{new_table_name}{feature_group_id[1]}"
                )
                final_old_table_name = (
                    f"{app_name}{app_name}{old_table_name}{feature_group_id[1]}"
                )
                ...
                with connection.cursor() as cur:
                    ...
                    try:
                        cur.execute(
                            f"INSERT INTO public.{final_new_table_name} (admission_no, physics, chemistry, maths) SELECT admission_no, physics, chemistry, maths FROM public.{final_old_table_name} WHERE admission_no = %s",
                            [students.admission_no],
                        )
                        cur.execute(
                            f"DELETE FROM public.{final_old_table_name} WHERE admission_no = %s",
                ...
                final_new_table_name = (
                    f"{app_name}{app_name}{new_table_name}{feature_group_id[2]}"
                )
                final_old_table_name = (
                    f"{app_name}{app_name}{old_table_name}{feature_group_id[2]}"
                )
                ...
                with connection.cursor() as cur:
                    ...
                    try:
                        cur.execute(
                            f"INSERT INTO public.{final_new_table_name} (admission_no, month_year_number, date, status) SELECT admission_no, month_year_number, date, status FROM public.{final_old_table_name} WHERE admission_no = %s",
                            [students.admission_no],
                        )
                        cur.execute(
                            f"DELETE FROM public.{final_old_table_name} WHERE admission_no = %s",
                ...
                final_new_table_name = (
                    f"{app_name}{app_name}{new_table_name}{feature_group_id[3]}"
                )
                final_old_table_name = (
                    f"{app_name}{app_name}{old_table_name}{feature_group_id[3]}"
                )
            ...
            return Response(
                {"Message": "Record successfully updated"}, status=status.HTTP_200_OK
            )
...
class ClassCreatTables(APIView):
    ...
    def post(self, request):
        try:
            ...
            table_name = (
                f"{data['batch_year']}_{data['class_name']}_{data['division']}".replace(
                    " ", ""
                ).lower()
            )
            ...
            return Response(
                {"message": "Table created successfully"}, status=status.HTTP_200_OK
            )
        except Exception as e:
            print(e)
            return Response(
                {"message": "Internal failure"}, status=status.HTTP_400_BAD_REQUEST
            )
...
class ClassMethods(APIView):
    ...
    def post(self, request):
        try:
            ...
            if serializer.is_valid():
                if class_details.objects.filter(
                    ...
                    table_name = f"{batch_year}_{class_name}_{division}".replace(
                        " ", ""
                    ).lower()
                    ...
                    if table_name:
                        ...
                        return Response(
                            {
                                "message": "Successfully created class group and associated feature group"
                            },
                            status=status.HTTP_201_CREATED,
                        )

                    return Response(
                        {"message": "Table names not valid"},
                        status=status.HTTP_201_CREATED,
                    )
            else:
                return Response(
    ...
    def put(self, request):
        try:
            ...
            if class_instance:
                ...
                table_name = f"{app_name}{app_name}{class_instance.batch_year}_{class_instance.class_name}_{class_instance.division}".replace(
                    " ", ""
                ).lower()
                ...
                new_table_name = f"{app_name}{app_name}{new_batch_year}_{new_class_name}_{new_division}".replace(
                    " ", ""
                ).lower()
                # Convert data values to uppercase
                for key in data:
                ...
                all_class_updates = class_updates_link.objects.filter(
                    class_name=class_instance.class_name,
                    batch_year=class_instance.batch_year,
                    division=class_instance.division,
                )
                ...
                all_recordings = recordings.objects.filter(
                    class_name=class_instance.class_name,
                    batch_year=class_instance.batch_year,
                    division=class_instance.division,
                )
                ...
                serializer = ClassDetailsSerializer(
                    class_instance,
                    data={
                        "batch_year": new_batch_year,
                        "class_name": new_class_name,
                ...
                if serializer.is_valid():
                    ...
                    for row in feature_group_id:
                        try:
                            cursor = connection.cursor()
                            cursor.execute(
                                f"ALTER TABLE {table_name+row} RENAME TO {new_table_name+row}"
                            )
                            print(f"Table {table_name+row} deleted")

                            students_instance = Student.objects.filter(
                                class_group=class_instance.id
                            )
    ...
    def get(self, request):
        try:
            ...
            if class_id:
                ...
                return Response(
                    {"class_details": serialized_data}, status=status.HTTP_200_OK
                )
        except Exception as e:
            print(e)
            return Response(
    ...
    def delete(self, request):
        ...
        if not class_id:
            return Response(
                {"message": "Class ID not provided"}, status=status.HTTP_400_BAD_REQUEST
            )

        class_instance = class_details.objects.filter(id=class_id).first()
        if not class_instance:
            return Response(
                {"message": "Class group does not exist"},
                status=status.HTTP_400_BAD_REQUEST,
            )

        app_name = "register_student_"
        table_name = f"{class_instance.batch_year}_{class_instance.class_name}_{class_instance.division}".replace(
            " ", ""
        ).lower()
        feature_group_id = [
            "_examresults",
...
class StudentBulkMethods(APIView, CustomPageNumberPagination):
        ...
        try:
            ...
            if file_obj:
                ...
                serializer = StudentSerializer(
                    data=data.to_dict(orient="records"), many=True
                )
                ...
                if serializer.is_valid():
                    ...
                    return Response(
                        {"message": "Successfully added users to class group"},
                        status=status.HTTP_201_CREATED,
                    )
    ...
    def get(self, request):
        try:
            ...
            if serializer.data == []:
                return Response(
                    {"Message": "No records found"}, status=status.HTTP_400_BAD_REQUEST
                )
