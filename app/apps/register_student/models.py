...
def create_dynamic_models(model_names):
    try:
        ...
        for base_model, new_model_name in zip(base_models, model_names):
            ...
            fields = {field.name: field.clone() for field in base_model._meta.fields}
...
def create_tables(app_name, unique_table_names):
    print(app_name)
    model_names = [
        app_name + unique_table_names + "_examresults",
        app_name + unique_table_names + "_leaderboard",
        app_name + unique_table_names + "_attendance",
        app_name + unique_table_names + "_dailyupdates",
    ]
